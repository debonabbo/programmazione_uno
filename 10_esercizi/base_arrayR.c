...
void stampa_rev_arrayR(const int a[], const size_t i);
                   ...
                   const size_t bLen, const int b[], const size_t i)
{
    ...
    if(aLen != bLen || a[i] != b[i]){
        ret = false;    //Fine
    } else if(i >= aLen){
        //Fine
    } else {
        ret = array_ugualiR(aLen, a, bLen, b, i+1);
    }
                      ...
                      const int val, const size_t i)
{
    ...
    if(i >= aLen){
        ...
        ret = esiste_multiploR(aLen, a, val, i+1);
    }
