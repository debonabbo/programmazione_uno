...
int prodotto_multipli_in_intervalloR(int n_min, int n, int k){
	...
	if(n > n_min){	//Passo induttivo
		prodotto *= prodotto_multipli_in_intervalloR(n_min, n-1, k);
	}
...
bool esiste_divisore_in_intervalloR(int n_min, int n, int k){
	...
	if(n >= n_min){
		if(k%n==0){		//Caso base
			...
			esiste = esiste_divisore_in_intervalloR(n_min, n-1, k);
		}
	}
