...
bool trova_carattereR(const char* pStr, char ch){
	...
	if(*pStr == ch){	//Caso base
		ret = true;
	} else {			//Caso generale
		if(*pStr != '\0')
		ret = trova_carattereR(pStr+1, ch);
	}
...
int conta_caratteriR(const char* pStr, char ch){
	...
	if(*pStr == '\0'){	//Caso base
		...
		if(*pStr == ch)
			totale++;
		totale += conta_caratteriR(pStr+1, ch);
	}
...
bool tutte_maiuscoleR(const char* pStr){
	...
	if(islower(*pStr)){	//Caso base
		ret = false;
	} else {	//Caso generale
		if(*pStr != '\0')
			ret = tutte_maiuscoleR(pStr+1);
	}
...
void filtra_carattere(char* pStr, char ch){
	char* pDest = pStr;
	filtra_carattereR(pStr, pDest, ch);
}
...
//    poi scrivi ch in *pDest e restituisce pDest+1
void capovolgi_str(char* pStr){
	char* pStrInizio = pStr;
	capovolgi_strR(pStr, pStrInizio);
}
